...
const Button: React.FC<IButtonProps> = ({ width, cstyle, text, ...rest }) => {
  ...
  return (
    <div {...rest} className={`${customStyle} cursor-pointer flex justify-center items-center rounded-lg py-[14px] max-w-[240px] w-full text-base font-bold btn-shadow transition-all hover:scale-105`}>
      {text}
    </div>
  );
};
